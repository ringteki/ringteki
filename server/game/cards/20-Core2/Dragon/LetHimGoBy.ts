    ...
    public setupCardAbilities() {
        this.duelFocus({
            ...
            target: {
                ...
                cardCondition: (card, context) => card.parent && context.event.duel.isInvolved(card.parent) && this.getAttachmentSkill(card, context) !== 0,
                gameAction: AbilityDsl.actions.cardLastingEffect(context => ({
                    target: context.target.parent,
                    effect: AbilityDsl.effects.modifyDuelSkill(this.getAttachmentSkill(context.target, context), context.event.duel),
        ...
        this.reaction({
            ...
            when: {
                ...
                onCardPlayed: (event, context) => event.card.isParticipating() && context.source.controller !== context.player
            },
