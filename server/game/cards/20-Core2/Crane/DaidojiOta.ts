import { CardTypes, Players, TargetModes, Locations } from '../../../Constants';
...
export default class DaidojiOta extends DrawCard {
    ...
    setupCardAbilities() {
        this.persistentEffect({
            ...
            condition: (context) => context.player.cardsInPlay.any(card => card.getType() === CardTypes.Character && card.costLessThan(3)),
            effect: AbilityDsl.effects.reduceCost({
                amount: (card, player) => {
                    ...
                    return -1 * (dynastyMatchesByName.length + conflictMatchesByName.length);
                },
        ...
        this.action({
            ...
            effect: 'make {1}{2}',
            effectArgs: (context) => context.select === 'Discard an event' ? 
