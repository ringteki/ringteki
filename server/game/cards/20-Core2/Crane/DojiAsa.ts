...
export default class DojiAsa extends DrawCard {
    ...
    setupCardAbilities() {
        this.action({
            ...
            condition: (context) =>
                context.game.currentConflict.getParticipants().some((p) => p.controller !== context.player),
            effect: '{1}',
            effectArgs: (context) =>
                context.select === 'Give opponent +3 skill'
                ...
                choices: {
                    ...
                    'Duel to try and cancel': AbilityDsl.actions.selectCard((context) => ({
                        ...
                        gameAction: AbilityDsl.actions.duel({
                            ...
                            message: '{0}{1}{2}',
                            messageArgs: (duel) =>
                                duel.winnerController === context.player
