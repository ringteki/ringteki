...
class CaptivatingStory extends DrawCard {
    setupCardAbilities() {
        this.action({
            ...
            condition: context => this.game.currentConflict.getNumberOfParticipantsFor(context.source.controller) === 1,
            target: {
                cardType: CardTypes.Character,
                controller: Players.Self,
                cardCondition: (card, context) => card.isParticipating() && (context.player.getNumberOfFaceupProvinces() > 0 || card.fate > 0),
