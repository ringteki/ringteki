...
class Subterfuge extends DrawCard {
    setupCardAbilities() {
        this.wouldInterrupt({
                            ...
                            AbilityDsl.actions.handler({
                                handler: (context) => {
                                    if(!this.messageShown) { // for some reason, it shows the message twice
                                        context.game.addMessage('{0} discards {1}', context.player.opponent, cardsToDiscard);
                                        if(drawAmount > 0) {
                                            context.game.addMessage('{0} draws {1} card{2}', context.player.opponent, drawAmount, drawAmount > 1 ? 's' : '');
                                        }
            ...
            effectArgs: context => [Math.min(context.event.amount, 3), context.event.amount > 1 ? 's' : '', context.event.amount > 1 ? 'them' : 'it']
        });
    }
}
