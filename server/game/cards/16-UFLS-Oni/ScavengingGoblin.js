...
class ScavengingGoblin extends BaseOni {
    setupCardAbilities() {
                        ...
                        AbilityDsl.actions.handler({
                            handler: (context) => {
                                if(!this.messageShown) { // for some reason, it shows the message twice
                                    context.game.addMessage('{0} {1} removed from the game from the top of {2}\'s conflict deck', cardsToRemove, cardsToRemove.length > 1 ? 'are' : 'is', context.player.opponent);
                                    if(attachmentsToRemove.length > 0) {
                                        context.game.addMessage('{0} {1} removed from the game due to sharing a name with a card that was removed from the deck', attachmentsToRemove, attachmentsToRemove.length > 1 ? 'are' : 'is');
                                    }
                                }
                            }
                        })
...
ScavengingGoblin.id = 'scavenging-goblin';

module.exports = ScavengingGoblin;
