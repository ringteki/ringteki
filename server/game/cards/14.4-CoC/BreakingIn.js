...
class BreakingIn extends ProvinceCard {
    setupCardAbilities() {
        this.reaction({
                    ...
                    if(cardFromDeck.hasTrait('cavalry')) {
                        return this.game.promptForSelect(context.player, {
                            ...
                            onSelect: (player, card) => {
                                this.game.addMessage('{0} places {1} in {2}', context.player, cardFromDeck, card.facedown ? card.location : card);
...
BreakingIn.id = 'breaking-in';

module.exports = BreakingIn;
