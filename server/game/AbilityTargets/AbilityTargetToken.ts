...
export class AbilityTargetToken extends AbilityTargetBase {
    ...
    constructor(
        ...
        // @TODO always set it to true. Remove this?
    ...
    getSelector(properties) {
        ...
        let cardType = properties.cardType || [
        ...
        return CardSelector.for(
            Object.assign({}, properties, { cardType: cardType, cardCondition: cardCondition, targets: false })
        );
    }
    ...
    resolve(context, targetResults) {
        ...
        let promptProperties = {
            ...
            onSelect: (player, card) => {
                ...
                let validTokens = card.statusTokens.filter(
                    (token) =>
                        (!this.properties.tokenCondition || this.properties.tokenCondition(token, context)) &&
                        (this.properties.gameAction.length === 0 ||
                            this.properties.gameAction.some((action) => action.canAffect(token, context)))
                );
    ...
    checkTarget(context) {
        if (
            !context.tokens[this.name] ||
            context.tokens[this.name].length === 0 ||
