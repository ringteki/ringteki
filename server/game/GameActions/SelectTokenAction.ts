...
export class SelectTokenAction extends TokenAction {
    ...
    canAffect(token: StatusToken, context: AbilityContext, additionalProperties = {}): boolean {
        ...
        return super.canAffect(token, context) && properties.tokenCondition(token, context) &&
            properties.gameAction.hasLegalTarget(context, Object.assign({}, additionalProperties, properties.subActionProperties(token)));
    }
    ...
    addEventsToArray(events, context: AbilityContext, additionalProperties = {}): void {
        ...
        let validTokens = properties.card.statusTokens.filter(token => properties.gameAction.canAffect(token, context));
        if(properties.singleToken && validTokens.length > 1) {
            ...
            properties.gameAction.addEventsToArray(events, context, Object.assign({}, additionalProperties, properties.subActionProperties(validTokens)));
        }
    }
